...
def Move(robot_i,rpm_l,rpm_r):
    ...
    return robot_n,heuristic_cost, movement_cost
...
def Move01(node):
    ...
    new_node = [heuristic_cost,NODEINDEX,node[1],new_robot,new_c2c]
    return new_node
...
def a_star_algo(start_node,goal_node,c2c_node):
    ...
    OpenList = []
    ClosedList = []
    ClosedList.append(((Cost_Calc(start_node[0],start_node[1])),NODEINDEX,0,start_node,0))
    hq.heappush(OpenList,((Cost_Calc(start_node[0],start_node[1])),NODEINDEX,0,start_node,0))  #Pushes starting node into OpenList
    iterator = 0
    cv.waitKey(0)
